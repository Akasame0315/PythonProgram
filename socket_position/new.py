...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.direction = 0
        self.image = pygame.transform.scale(plane01_img, (150, 120)) #調整圖片大小
    ...
    while connected:
        ...
        if msg_length:
            ...
            if msg != DISCONNECT_MESSAGE:
                ...
                write_text(screen,"ClientPosx:" + str(newMsg[0]), 22, 100, 70)
                write_text(screen,"ClientPosy:" + str(newMsg[1]), 22, 100, 90)

                conn.send(f"{ServerX} , {ServerY}".encode(FORMAT))
                cx = newMsg[0]
                cy = newMsg[1]
                # conn.send(", ".encode(FORMAT))
                # conn.send(f"{ServerY}".encode(FORMAT))
